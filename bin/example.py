...
if __name__ == "__main__":
    ...
    with Session(engine) as session:
        ...
        for depot in scenario.depots:
            ...
            fig.write_html(os.path.join(DEPOT_OUTPUT_DIR, "power_and_occupancy.html"))
            fig.show()
            ...
            for color_scheme in "event_type", "soc", "location":
                ...
                fig.write_html(
                    os.path.join(DEPOT_OUTPUT_DIR, f"depot_event_{color_scheme}.html")
                )
                fig.show()
            ...
            for vehicle in vehicles:
                ...
                fig.write_html(
                    os.path.join(VEHICLE_OUTPUT_DIR, f"vehicle_{vehicle.id}_soc.html")
                )
                # fig.show()
