...
class InputReader:
    LINK_INTRADAY = "https://www.epexspot.com/en/market-data/intradaycontinuous/intraday-table/"
    LINK_DAY_AHEAD = "https://www.epexspot.com/en/market-data/dayaheadauction/auction-table/"
    COUNTRY_INTRADAY = "/DE"
    ...
    def input_day_ahead(self):
        ...
        while date <= self.end_date:
            raw_pd_frame = pd.read_html(InputReader.LINK_DAY_AHEAD+ str(date) + InputReader.COUNTRY_DAY_AHEAD)[8]
            ...
            while i < 9:
                ...
                date_name = str(date + datetime.timedelta(days=(i-8))) #because of the +6 on top and the structure of the pd_frame it has to be +8
    ...
    def input_intraday(self):
        ...
        date = self.start_date
        vector_row_needed = [2, 3, 5, 6, 9, 10, 12, 13, 16, 17, 19, 20, 23, 24, 26, 27, 30, 31, 33, 34, 37, 38, 40, 41, 44, 45, 47, 48, 51, 52, 54, 55, 58, 59, 61, 62, 65, 66, 68, 69, 72, 73, 75, 76, 79, 80, 82, 83, 86, 87, 89, 90, 93, 94, 96, 97, 100, 101, 103, 104, 107, 108, 110, 111, 114, 115, 117, 118, 121, 122, 124, 125, 128, 129, 131, 132, 135, 136, 138, 139, 142, 143, 145, 146, 149, 150, 152, 153, 156, 157, 159, 160, 163, 164, 166, 167]
        while date < (self.end_date + datetime.timedelta(days=1)) :
            date += datetime.timedelta(days=1)

            raw_pd_frame = pd.read_html(InputReader.LINK_INTRADAY+str(date)+InputReader.COUNTRY_INTRADAY)[0]
            values = raw_pd_frame.iloc[vector_row_needed,7].astype("float")/1000
            index = raw_pd_frame.iloc[vector_row_needed,2]
            series = pd.Series(list(values), index=index)
            pickle.dump(series, open(self.safe_path + str(date-datetime.timedelta(days=1)) + ".p", "wb"))
...
class PowerFrame:
    ...
    def pdframe(self):
        ...
        while time_in_sec <= self.max_sim_time:
            power_price = pickle.load(open(self.price_data_path+str(current_day)+".p","rb"))
