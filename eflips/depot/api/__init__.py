...
def generate_depot_layout(
    ...
    with create_session(scenario, database_url) as (session, scenario):
        ...
        for (
            ...
            if len(diesel_vt) > 0:
                num_tanking_slots = max(len(diesel_vt) // 10, 1)
            else:
                num_tanking_slots = None
            ...
            generate_depot(
                ...
                num_tanking_slots=num_tanking_slots,
            )
    ...
    with create_session(scenario, database_url) as (session, scenario):
        ...
        for depot_station, capacities in depot_capacities_for_scenario.items():
            ...
            if len(diesel_rotations) > 0:
                estimated_tanking_slot_count = estimate_service_capacity(
                    diesel_rotations, timedelta(minutes=15)
                )
            else:
                estimated_tanking_slot_count = None
            ...
            generate_depot(
                ...
                num_tanking_slots=estimated_tanking_slot_count,
            )
