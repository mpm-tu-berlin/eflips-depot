...
def _resolve_entry_filter_and_remap_processes(
    ...
    if area.vehicle_type_id is not None:
        ...
        # TODO: tanking workaround should use a proper tag/attribute, not a name check
        if area.name and "tanking" in area.name.lower():
            allowed_vt_ids = [
                str(vt.id)
...
def _build_process_entries(processes: list, scenario: Scenario, template: dict) -> None:
    ...
    for process in processes:
        ...
        # TODO: tanking workaround — replace with a proper attribute/tag on the process
        if process.name and "tanking" in process.name.lower():
            base["vehicle_filter"] = {
