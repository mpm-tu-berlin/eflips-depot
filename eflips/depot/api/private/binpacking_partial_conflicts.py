...
def best_possible_packing_parcial(session, depot, bin_width=None, bin_height=None):
   ...
   # Unpacking
    ...
    # ----------------------------------------------------------
    placed_areas,driving_lanes,final_width,final_length,available_spaces = last_success
    ...
    visualize_placements(placed_areas, driving_lanes, final_width, final_length, session)
...
def retrieve_depot_capacities(
    ...
    charging_process = session.query(Process).filter(Process.name == "Charging",Process.scenario_id == depot.scenario_id).one_or_none()
    ...
    areas = (session.query(Area).join(Area.processes).filter(Area.depot_id == depot.id,Process.id == charging_process.id).all())
    ...
    for area in areas:
        ...
        if area.area_type == AreaType.LINE:
            ...
            results[vehicletype]["direct_capacity"] = area.capacity  # It is assumed that there is only one DIRECT area per VehicleType.
...
def bin_packing(session,depot,bin_width=None,bin_height=None):
    ...
    for vehicle_type, estimate in dict_results.items():
        ...
        if direct_parking_slots > 0:
            ...
            height = x*math.sin(math.radians(45))+z*math.sin(math.radians(45)) + (direct_parking_slots-1) * z/math.cos(math.radians(45))
    ...
    area_without_dirveways= sum(width * height for _, width, height, _ in parking_areas)
    ...
    areas_sorted = sorted(parking_areas, key = lambda r: (0, -r[1], -r[2]) if not r[3] else (1, -r[2], -r[1]))
    ...
    available_spaces = [(inner_edge,inner_edge,bin_width-2*inner_edge,bin_height-2*inner_edge)] # (x,y,breite,hoehe)
    placed_areas = []
    driveways = []
    ...
    for area in areas_sorted:
        ...
        placed = placing_areas_on_parking_space(area,available_spaces,driveways,placed_areas)
        
        if not placed:
            print(f"{area} konnte nicht platziert werden. Abbruch des 'Bin Packing'-Algorithmus")
            return None 
...
def placing_areas_on_parking_space(area, available_spaces,driveways, placed_areas):
    ...
    for i, available in enumerate(available_spaces):
        ...
        if free_h >= area_h  and free_w >= area_w :
            ...
            if drivinglane_necessary['left_driveway'] == True or drivinglane_necessary['same_height'] == True:
               # If a sufficiently large driveway exists, the parking space can be placed
                place_area_and_update(area,available_spaces,best_fit_index, driveways, drivinglane_necessary, placed_areas)
            ...
            # In case no driveway is adjacent
                ...
                if enough_space_for_area_and_driveway(area, parking_area, drivinglane_necessary):
...
def driveway_check(area, parking_area, driveways):
    ...
    if not is_line:
        ...
        required_gap = conflict_matrix.get((rect_type, 'top'), 0) # Assuming that top and bottom gaps are of equal size
    ...
    if not is_line:
        ...
        for driveway in driveways:
            ...
            if (driveway_x + driveway_w == x) and (driveway_y <= y) and (driveway_y + driveway_h >= y + area_h):
                left_driveway = True
                break
    ...
    # Second check for 'line' parking spaces 
        ...
        for driveway in driveways:
            ...
            if (driveway_y == y + free_h) and (driveway_x <= x) and (driveway_x + driveway_w >= x + area_w):
                upper_driveway = True
            
            # Check if a driveway is adjacent below the parking space and if the width is sufficient
            if (driveway_y + driveway_h == y) and (driveway_x <= x) and (driveway_x + driveway_w >= x + area_w):
                lower_driveway = True 
...
def place_area_and_update(area, available_spaces, best_fit_index, driveways, drivinglane_necessary, placed_areas):
    ...
    if not is_line:
        ...
        required_gap = conflict_matrix.get(('block', 'top'), 0) # Assuming that top and bottom gaps are of equal size
    ...
    if left_driveway or same_height:
        ...
        new_spaces = get_newfree_rectangles(area_and_driveway_combination, available_spaces, best_fit_index)
    ...
    if rect_type=='dsr' and not left_driveway:
        ...
        placed_areas.append((vehicle_type, x + required_gap, y, area_w, area_h , is_line))
    ...
    if rect_type == 'line':
        ...
        # and the driveway above is used
            ...
            # 3) Placement of the parking space 
            placed_areas.append((vehicle_type, x, y + required_gap, area_w, area_h, is_line))
...
def get_newfree_rectangles(area_and_driveway_combination, available_spaces, best_fit_index, min_size=2):
    """
    ...
    if not (area_x_right <= free_x or area_x >= free_x_right or
            area_y_low <= free_y or area_y >= free_y_low):
        ...
        if area_x > free_x:
            ...
            if neue_breite >= min_size:
                free_rects.append((free_x, area_y, neue_breite, min(area_h, free_y_low - area_y)))

        # Right rectangle
        if area_x_right < free_x_right:
            neue_breite = free_x_right - area_x_right
            if neue_breite >= min_size:
                free_rects.append((area_x_right, area_y, neue_breite, min(area_h, free_y_low - area_y)))
...
def enough_space_for_area_and_driveway(area, parking_area, drivinglane_necessary):
    ...
    if not is_line:
        ...
        if upper_driveway_used:
            if free_w >= area_w and math.isclose(free_h, area_h + required_gap, rel_tol = 1e-6):
                return True 
...
def visualize_placements(placements, driveways, container_width, container_height, session):
    """
    ...
    legend_.append(patches.Patch(facecolor='black', edgecolor='black', hatch = '/', alpha=0.3, label = "Fahrwege"))
