...
class Validator:
    ...
    def all_periods(self, periods_map):
        ...
        # Run specific tests
        self._park_inside_depot(vehicledata,
                                name_general=periods_map['depot general'],
                                name_park=periods_map['park'])
        self._charge_inside_park(vehicledata,
                                 name_park=periods_map['park'],
                                 names_charge=periods_map['charge'])
        self._service_outside_park(vehicledata,
                                   name_park=periods_map['park'],
                                   name_serve=periods_map['serve'])
        self._trip_outside_depot(vehicledata,
                                 name_general=periods_map['depot general']
                                 )
    ...
    def _park_inside_depot(self, vehicledata, name_general, name_park):
        ...
        for vID in vehicledata:
            ...
            for park_start, park_dur in parks:
                for general_start, general_dur in generals:
                    if park_start >= general_start and park_start + park_dur <= general_start + general_dur:
                        found[i] = True
                        i += 1
                        break
            result[vID] = found
        ...
        self.results['park_inside_depot'] = {
            'valid': valid,
            'result': result
        }
    ...
    def _charge_inside_park(self, vehicledata, name_park, names_charge):
        ...
        for vID in vehicledata:
            ...
            for charge_start, charge_dur in charges:
                for park_start, park_dur in parks:
                    if charge_start >= park_start and charge_start + charge_dur <= park_start + park_dur:
                        found[i] = True
                        i += 1
                        break
            result[vID] = found
        ...
        self.results['charge_inside_park'] = {
            'valid': valid,
            'result': result
        }
    ...
    def _service_outside_park(self, vehicledata, name_park, name_serve):
        ...
        for vID in vehicledata:
            ...
            for serve_start, serve_dur in serves:
                for park_start, park_dur in parks:
                    ...
                    if park_start < serve_start < park_end or park_start < serve_start + serve_dur < park_end:
                        not_found[i] = False
                        i += 1
                        break
        ...
        self.results['service_outside_park'] = {
            'valid': valid,
            'result': result
        }
    ...
    def _trip_outside_depot(self, vehicledata, name_general):
        ...
        for vID in vehicledata:
            ...
            for trip in trips:
                for general_start, general_dur in generals:
                    ...
                    if general_start < trip.atd < general_end or general_start < trip.ata < general_end:
                        not_found[i] = False
                        i += 1
                        break
            result[vID] = not_found
        ...
        self.results['trip_outside_depot'] = {
            'valid': valid,
            'result': result
        }
