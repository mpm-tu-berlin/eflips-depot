...
from eflips.simpy_ext import FilterStoreExt, PositionalFilterStore, \
    StorePutExt, LineFilterStore, StoreConnector, FilterStoreExtGet, \
    LineStorePut, LineFilterStoreGet, ExclusiveRequest
...
from eflips.depot.rating import SlotAlternative, ParkRating, \
    VehicleAlternative, DispatchRating
...
class DepotWorkingData:
    ...
    def current_charging_interface(self):
        ...
        """
        if self.current_area is not None \
                and self.current_area.charging_interfaces:
    ...
    def isrfd(self):
        ...
        trip requirements.
        """
        return self.current_area is not None and self.current_area.issink \
...
class BackgroundStorePut(StorePut):
    ...
    def __init__(self, store, item):
        ...
        # Logging
        if hasattr(store, 'logger') and globalConstants['general'][
...
class Depot:
    ...
    def overdue_trips(self):
        ...
        a scheduled vehicle.
        """
        return [trip for trip in self.pending_departures if trip.atd is None
                and trip.std < self.env.now]
    ...
    def assign(vehicle, trip, depot):
        ...
        if vehicle.dwd.any_active_processes:
            ...
            flexprint(
                ...
                % (trip.ID, vehicle.ID, vehicle.battery.soc,
                   vehicle.dwd.active_processes,
                   vehicle.dwd.any_active_processes, vehicle.dwd.on_hold,
                   VehicleFilter(filter_names=['isunblocked'])(vehicle),
                   trip.std, vehicle.dwd.current_area,
        ...
        preconddata = next(
            (depot.processes[proc_ID] for proc_ID, procdata in depot.processes.items()
             if proc_ID in vehicle.dwd.current_area.available_processes
             and issubclass(procdata['type'], Precondition)),
            None)

        if preconddata is not None:# and preconddata['kwargs']['vehicle_filter'](vehicle):
            ...
            if proc_obj.vehicle_filter(vehicle):    # Apply filters
                ...
                if until_std > 0:
                    if until_std > proc_obj.dur:
                        start_delayed(env, proc_obj(vehicle=vehicle),
                                      until_std - proc_obj.dur)
                    else:
        ...
        flexprint(
            ...
            % (vehicle.ID, vehicle.vehicle_type.ID, trip.ID,
               trip.vehicle_types),
            env=env, switch='dispatch2')
...
class DSFirst(BaseDispatchStrategy):
    ...
    tooltip = 'Match trips and vehicles in the order of creation of the ' \
        ...
        for parking_area_group in depot.parking_area_groups:
            ...
            for store in parking_area_group.stores:
                ...
                if isinstance(store, LineArea):
                    ...
                    vehicles = [store.items[i] for i in rg
                                if store.items[i] is not None]
                else:
                ...
                if len(vehicles) > 0:
                    ...
                    pending_vehicles = [
                        v for v in vehicles if v.trip is None
                        or trip.std < v.trip.std
                    ]
                    for v in vehicles:
                        if v.trip is not None and trip.std < v.trip.std:
                            flexprint('reconsidering vehicle %s: trip %s std %d is later than trip %s std %d'
    ...
    def find_match(depot):
        ...
        if next_trips:
            ...
            for next_trip in next_trips:
                ...
                pending_vehicles = DSFirst.get_pending_vehicles(depot,
                                                                next_trip)
                for parking_area_group in pending_vehicles:
                    for store in pending_vehicles[parking_area_group]:

                        vehicle = pending_vehicles[parking_area_group][
                            store][0]
                    ...
                    for direct_area in depot.direct_departure_areas:
                        for vehicle in direct_area.vehicles:
                            if vehicle.trip is None \
                                    and DSFirst.try_assign(vehicle, next_trip, 
                                                           depot):
                                # Recall because the assignment situation has
                                # changed
                                DSFirst.trigger(depot)
            ...
            if depot.any_process_cancellable_for_dispatch \
                ...
                for next_trip in next_trips:
                    ...
                    urgent = next_trip.vehicle is None and next_trip.std <= next_trip.env.now
                    if urgent and not next_trip.periodic_trigger_scheduled:
                        depot.env.process(DSFirst.trigger_until_found(
                            depot, next_trip, globalConstants['depot']['dispatch_retrigger_interval']))
...
class DSSmart(BaseDispatchStrategy):
    ...
    vf_urgent = VehicleFilter(filter_names=[
        'vehicle_type',
        'not_on_hold',
        'no_active_uncancellable_processes',
        'sufficient_energy',
    ...
    vf_usual = VehicleFilter(filter_names=[
        'vehicle_type',
        'not_on_hold',
        'no_active_processes',
        'sufficient_energy']
        ...
        for parking_area_group in depot.parking_area_groups:
            for area in parking_area_group.stores:
                ...
                if isinstance(area, LineArea):
                    ...
                    vehicle = next(
                        (area.items[idx] for idx in rg
                         if area.items[idx] is not None
                         and (area.items[idx].trip is None
                    ...
                    for vehicle in area.vehicles:
                        if (vehicle.trip is None or trip.std < vehicle.trip.std) and vf(vehicle):
                            vehicles.append(vehicle)
    ...
    def find_match(depot):
        ...
        for trip in next_trips:
            ...
            if vehicles:
                ...
                depot.env.process(DSSmart.trigger_until_found(depot, trip, globalConstants['depot']['dispatch_retrigger_interval']))
    ...
    def scheduling_delay(env, trip):
        ...
        """
        if trip.std < env.now:
            raise RuntimeError('scheduling_delay cannot be determined if the '
...
class DepotControl:
    ...
    """
    dispatch_strategies = {
        DSFirst.name: DSFirst,
        DSSmart.name: DSSmart
    }
    ...
    def dispatch_strategy(self, value):
        ...
        self._dispatch_strategy_name = next(
            name for name, cls in self.dispatch_strategies.items()
            if cls is value)
    ...
    def checkin(self, vehicle):
        ...
        flexprint('%s checking in at %s with battery level = %f'
    ...
    def proceed(self, vehicle):
        ...
        if vehicle.dwd.plan and not issink:
            ...
            if isinstance(next_entry, AreaGroup):
                self.env.process(self.proceed_group(vehicle, current_area,
                                                    next_entry))
            else:
                self.env.process(self.proceed_area(vehicle, current_area,
                                                   next_entry))
        else:
    ...
    def proceed_area(self, vehicle, current_area, next_area):
        ...
        flexprint('Vehicle %s calling at area %s'
                  % (vehicle.ID, next_area.ID), env=self.env,
                  switch='operations')

        # Check if the vehicle meets entry condition for the next area.
        permission = next_area.entry_filter(vehicle)
        if permission:
            flexprint('%s entry at area %s permitted. Vehicle type is %s.'
            ...
            flexprint('%s requests %s at area %s'
                      % (vehicle.ID, process_IDs, next_area.ID), env=self.env,
                      switch='operations')
            ...
            if process_IDs:
                ...
                flexprint('vehicle %s was put into area %s.'
                          % (vehicle.ID, next_area.ID),
                          env=self.env, switch='operations')

                # Get vehicle from current area
                if current_area is not None:
                    yield current_area.get(
                        lambda item: item.ID == vehicle.ID)
    ...
    def proceed_group(self, vehicle, current_area, group):
        ...
        flexprint('Vehicle %s calling at AreaGroup %s'
                  % (vehicle.ID, group.ID), env=self.env, switch='operations')
        if globalConstants['depot']['log_cm_data']:
            self.depot.evaluation.cm_report.log(ProcessCalled(self.env, vehicle))
        ...
        flexprint(
            ...
            ' %s.' % (vehicle.ID, group.ID, permissions, 
        ...
        if any(permissions):
            ...
            flexprint('Vehicle %s process_IDs_all: %s' % (
                vehicle.ID, process_IDs_all), env=self.env, 
                      switch='operations')

            selection = [True if perm and pl else False for perm, pl in
                         zip(permissions, process_IDs_all)]
            flexprint('Vehicle %s selection: %s' % (vehicle.ID, selection),
                      env=self.env, switch='operations')
            # Initiate movement if any processes are needed
            if any(selection):
                ...
                vehicle.dwd.on_hold = True
                next_area = yield self.env.process(group.put(vehicle, 
                                                             selection))
                waiting_time = self.env.now - req_time
                if waiting_time > 0 and current_area is not None:
                    flexprint('Delay from %s to %s of %s secs! Vehicle: %s'
                              % (current_area.ID, group.ID, waiting_time,
                                 vehicle.ID),
                              env=self.env, switch='parking_full')
                    if self.parking_congestion_event_cls is not None \
                            and isinstance(group, ParkingAreaGroup):
                        pc_event = self.parking_congestion_event_cls(self.env)
                        pc_event.succeed()

                flexprint('Vehicle %s was put into area %s through group.'
                    % (vehicle.ID, next_area.ID), env=self.env,
                          switch='operations')
                ...
                if current_area is not None:
                    yield current_area.get(
                        lambda item: item.ID == vehicle.ID)
                    flexprint('Got vehicle %s from area %s through group.' % (
                        vehicle.ID, current_area.ID), env=self.env,
                              switch='operations')
    ...
    def get_process_need(self, vehicle, area):
        ...
        """
        flexprint('\tavailable_processes of area %s: %s' % (area.ID,
                       area.available_processes), env=self.env, switch='operations')

        # Append entries where the process is mandatory or both optional and
        # vehicle_filter returns True
        process_IDs = [
            procID for procID in area.available_processes
            if self.depot.processes[procID]['type'].request_immediately and (
    ...
    def run_processes(self, vehicle, process_IDs):
        ...
        finished.
        """
        flexprint('\t%s process list: %s' % (vehicle.ID, process_IDs), 
                  env=self.env, switch='operations')
        vehicle.dwd.any_active_processes = True
        vehicle.dwd.on_hold = False

        # Instantiate process objects
        proc_objs = [self.depot.processes[procID]['type'](
        ...
        flexprint('Vehicle %s starting processes %s' % (
                vehicle.ID, [pr.ID for pr in proc_objs]), env=self.env,switch='operations')

        yield simpy.AllOf(self.env, calls)

        flexprint('Vehicle %s finished processes %s' % (
                vehicle.ID, [pr.ID for pr in proc_objs]), env=self.env,switch='operations')
    ...
    def request_vehicle(self, trip, filter=lambda item: True):
        ...
        flexprint('Received request for trip %s with std = %d'
                  % (trip.ID, trip.std), env=self.env,
                  switch='timetable')
    ...
    def register_for_dispatch(self, trip):
        ...
        self.env.process(trip.notify_due_departure(
    ...
    def process_request_prio_parking(self, trip, filter=lambda item: True):
        ...
        self.trigger_dispatch()
        vehicle, requests = yield self.env.process(
            self.departure_areas.get_imm(filter))
        ...
        if vehicle is None:
            ...
            if match:
                ...
                flexprint('match found in depot init', env=self.env,
                          switch='dispatch')
                flexprint('vehicles left in init: %d'
                          % (self.depot.init_store.count),
                          switch='dispatch')
            else:
                # No vehicle available in depot.init_store, wait for the
                # success of a request in self.departure_areas
                flexprint('no immediate match in init_store',
                          switch='dispatch')
        ...
        flexprint(
            ...
            + '; current_area: %s'
            % (vehicle.dwd.current_area.ID if vehicle.dwd.current_area is not None else None),
            env=self.env, switch='objID', objID=vehicle.ID)
    ...
    def process_request_prio_init(self, trip, filter=lambda item: True):
        ...
        if trip.reserved_for_init:
            ...
            vehicle = yield self.env.process(
                self.departure_areas.get(filter=filter))
...
def update_relations(put_event):
    ...
    if hasattr(vehicle, 'logger') and globalConstants['general'][
        'LOG_ATTRIBUTES'] and globalConstants['general'][
        'LOG_SPECIFIC_STEPS']:
        vehicle.logger.steplog()
...
class BaseArea(ABC):
    ...
    *entry_filter* can be put on any slot, not considering e.g. vehicle
    ...
    """
    @abstractmethod
    def __init__(self, env, ID, available_processes, issink, entry_filter=None,
                 charging_interfaces=None, slot_orientation="HORIZONTAL",
                 *args, **kwargs):
        ...
        if charging_interfaces is not None:
            if not isinstance(charging_interfaces, list) \
                    or (charging_interfaces and len(charging_interfaces) != self.capacity) \
                    or not all(isinstance(i, DepotChargingInterface)
                               for i in charging_interfaces):
                raise ValueError(
        ...
        self.issink = issink
        self.entry_filter = entry_filter if entry_filter is not None \
            else VehicleFilter()
    ...
    def maxOccupiedSlots(self):
        if hasattr(self, "view"):
            return len([slotId for slotId in self.view.slotViews if self.view.slotViews[slotId].hasBeenUsed])
        return -1
    ...
    def istypestack_with(self, vehicle, substitution=True):
        ...
        *substitution* is True. Return None if the area is empty. Otherwise
        ...
        if substitution and vehicle.vehicle_type.group:
            return all(vi.vehicle_type in vehicle.vehicle_type.group.types
                       for vi in self.vehicles)
        else:
            return all(vi.vehicle_type is vehicle.vehicle_type
                       for vi in self.vehicles)
...
class BaseAreaPut(ABC):
    ...
    def __init__(self, store, *args, **kwargs):
        ...
        def print_after_put(request):
            flexprint('Vehicle %s moved to area %s' % (
                request.item.ID, request.item.dwd.current_area),
                      env=request.resource.env, switch="operations")
...
class DirectAreaPut(BaseAreaPut, ExclusiveRequest, StorePutExt):
    ...
    def __init__(self, store, item, other_requests=None):
        ...
        super(DirectAreaPut, self).__init__(store=store, item=item,
...
class DirectAreaGet(BaseAreaGet, ExclusiveRequest, FilterStoreExtGet):
    ...
    *filter* is a function receiving one item. It should return ``True`` for
    ...
    """
    def __init__(self, store, filter=lambda item: True, other_requests=None,
                 **kwargs):

        super(DirectAreaGet, self).__init__(store=store, filter=filter,
            other_requests=other_requests, **kwargs)
...
class DirectArea(BaseArea, PositionalFilterStore):
    ...
    """
    def __init__(self, env, ID, capacity, available_processes, issink,
                 entry_filter, charging_interfaces=None,
                 slot_orientation="HORIZONTAL"):
        super(DirectArea, self).__init__(
            env=env, ID=ID, capacity=capacity,
...
class LineAreaPut(BaseAreaPut, ExclusiveRequest, LineStorePut):
    ...
    *store*. The request is triggered once there is accessible space for the
    ...
    def __init__(self, store, item, side='default', other_requests=None):
        ...
        super(LineAreaPut, self).__init__(store=store, item=item, side=side,
                                          other_requests=other_requests)
...
class LineAreaGet(BaseAreaGet, ExclusiveRequest, LineFilterStoreGet):
    ...
    def __init__(self, store, filter=lambda item: True, side='default',
        ...
        super(LineAreaGet, self).__init__(store=store, filter=filter,
            side=side, other_requests=other_requests)
...
class LineArea(BaseArea, LineFilterStore):
    ...
    def __init__(self, env, ID, capacity, available_processes, issink,
                 entry_filter, charging_interfaces=None,
                 slot_orientation="VERTICAL", side_put_default='back',
        ...
        if not issink:
            warn('WARNING: LineArea should only be used as sinks, '
    ...
    def count_rfd_unblocked(self):
        ...
        blocked from departure at this area.
        """
        return sum(vehicle.dwd.isrfd and self.isunblocked(vehicle)
...
class PSFirst(BaseParkingStrategy):
    ...
    tooltip = 'This strategy will try to park a vehicle on the first ' \
...
class PSEven(BaseParkingStrategy):
    ...
    tooltip = 'This strategy will try to even out area usage inside a group,' \
...
class PSMixed(BaseParkingStrategy):
    ...
    tooltip = 'This strategy combines the FIRST and MIXED strategies, ' \
...
class PSSmart(BaseParkingStrategy):
    ...
    name = 'SMART'
    short_description = 'smart'
    tooltip = 'This strategy will try to utilize the areas inside a group ' \
              'according to\nupcoming departure requests, optimizing ' \
              'vehicle availability\n\nNote: Potential improvements only ' \
              'affect non-vehicle-typed parking lots.'

    @staticmethod
    def determine_store(preselected_stores, parking_area_group, item, *args,
                        **kwargs):

        # rate stores
        rated_stores = PSSmart.rate_stores(preselected_stores, item)
        best_rating = min(rated_stores, key=int) \
            if len(rated_stores) > 0 else -1

        # step 1A: try to use available slot on line store
        target_store = rated_stores[best_rating][0] \
            if best_rating > -1 else None
        ...
        if target_store is None or best_rating > 0:
            available_direct_stores = [
                direct_area for direct_area in parking_area_group.direct_areas
                if direct_area in preselected_stores
                and direct_area.vacant
            ]
        ...
        if not store.vacant_accessible:
            ...
            if len(pending_vehicles) > 0:
                ...
                if vehicle.vehicle_type.group is not None:
                    for otherVehicle in pending_vehicles:
                        if otherVehicle.vehicle_type is not vehicle.vehicle_type \
                                and otherVehicle.vehicle_type not in vehicle.vehicle_type.group.types:
                            typestack = False
                            break
                ...
                if typestack:
                    ...
                    while result < len(pending_vehicles) and pending_vehicles[
                            result].battery.energy < vehicle.battery.energy:
                                result = result + 1
                else:
...
class PSSmart2(BaseParkingStrategy):
    ...
    def determine_store(preselected_stores, parking_area_group, item, *args,
        ...
        preselected_stores = [store for store in preselected_stores
                              if store.vacant_accessible]
        if preselected_stores:
            rating = ParkRating(
                [SlotAlternative(
                    (store, store.index_put() if isinstance(store, LineArea)
...
class ParkingAreaGroup(AreaGroup):
    ...
    def stores(self, value):
        ...
        for store in value:
            if not store.issink:
                raise ValueError('issink must be True for all areas in a '
                                 'parking area group.')
    ...
    def parking_strategy(self, value):
        ...
        self._parking_strategy_name = next(
            name for name, cls in self.parking_strategies.items()
            if cls is value)
    ...
    def put(self, item, selection=None):
        ...
        preselected_stores = [st for st, flag in zip(self.stores, selection)
                              if flag]
        ...
        target_store = self.parking_strategy.determine_store(
            preselected_stores=preselected_stores, parking_area_group=self,
            item=item)
        ...
        if target_store is not None:
            ...
            flexprint('Calling put_imm for vehicle %s' % item.ID, env=self.env, switch='objID', objID=item.ID)
            store, requests = yield self.env.process(self.put_imm(item, selection))
            flexprint('\tput_imm result: %s. Len requests: %d' % (None if store is None else store.ID, len(requests)), switch='objID', objID=item.ID)
            if store is None:
...
class SpecificActivityPlan(BaseActivityPlan):
    ...
    """
    def __init__(self, ID, locations=None, vehicle_filter=None):
        super(SpecificActivityPlan, self).__init__(ID, locations,
                                                   vehicle_filter)
