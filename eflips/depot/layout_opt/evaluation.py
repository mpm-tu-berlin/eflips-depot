...
class OptEvaluation:
    ...
    """
    def __init__(self, pop, logbook, hof, crossovers, mutations,
                 memory, feasible, feasible_fr_vec):
        self.pop = pop
    ...
    def results_operators(self):
        ...
        if len(evals) == 1:
            ...
            return
        print('Crossovers total: %s; Average per gen: %s'
              % (sum(self.crossovers), np.average(self.crossovers)))
        mut_per_action = Counter(self.mutations)
        print('Successful mutations: %s; Total: %s; Average per gen: %s'
              % (mut_per_action, sum(mut_per_action.values()),
    ...
    def results_simtimes(self):
        ...
        print('Total simulations: %d, average computation time per simulation: %f seconds'
              % (len(simtimes), np.average(simtimes)))

    def results_feasbility(self):
        self._count_fitnesses()

        total_feasible = sum(self.feasiblity_counter[key] for key in self.feasiblity_counter if False not in key)
        ...
        print('Among evals total feasible: %d, total infeasible: %d, share of feasbile: %s, Counter: %s'
            % (total_feasible, total_infeasible, share_feasible, self.feasiblity_counter))
    ...
    def progress_so(self, objective_label='Kapazität'):
        ...
        minimum, maximum, avg, = self.logbook.chapters['fitness'].select(
            'min', 'max', 'avg')
        evals = self.logbook.select('evals')

        print('Gen %s: min=%s, max=%s, avg=%s'
    ...
    def progress_c_d_j(self, objectives_labels=('Kapazität', 'Verspätung [h]', 'Stau [h]'),
                       objectives_names=('c', 'd', 'j'),
                       show=True, save=False, formats=('pdf',),
        ...
        """
        return self.progress_mo(objectives_labels, objectives_names, show,
                                save, formats, basefilename)

    def progress_c_urfd(self, objectives_labels=('Kapazität', 'Unblocked ready for departure vehicles'),
                        objectives_names=('c', 'urfd'), show=True, save=False,
                        formats=('pdf',), basefilename='progress_c_urfd'):
        """Plots for the progress of the objectives values for depot layout
    ...
    def progress_mo(self, objectives_labels, objectives_names, show=True,
                    save=False, formats=('pdf',), basefilename='progress_mo'):
        """Plots for the progress of the objectives values for depot layout
    ...
    def objective_space_c_d_j(self, include_infeasible=True, show=True,
                              save=False, formats=('pdf',),
    ...
    def pareto_front_c_d_j(self, include_infeasible=True, show=True,
                           save=False, formats=('pdf',),
    ...
    def objective_space_c_urfd(self, include_infeasible=True, show=True,
                               save=False, formats=('pdf',),
                               basefilename='objective_space_c_urfd', xlim=('min', 'max')):
        """Plot the objective space based on found fitnesses of depot layout
        ...
        x_f, y_f, x_f_hof, y_f_hof, x_i_000, y_i_000, x_i_001, y_i_001, x_i_010, y_i_010, x_i_011, y_i_011, x_i_100, y_i_100,\
        x_i_101, y_i_101, x_i_110, y_i_110= ([] for _ in range(18))

        #Counter for verification
        count_f = count_hof = count_000 = count_001 = count_010 = count_011 = count_100 = count_101 = count_110 = 0
        ...
        print(count_f, count_hof, count_000, count_001, count_010, count_011, count_100, count_101, count_110)

        fig, ax = baseplot(show)

        if include_infeasible:
            sc_infea_000 = ax.scatter(x_i_000, y_i_000, marker='x', c='#969696', alpha=0.3)
            sc_infea_001 = ax.scatter(x_i_001, y_i_001, marker='x', c='#000096', alpha=0.3)
            sc_infea_010 = ax.scatter(x_i_010, y_i_010, marker='x', c='#009600', alpha=0.3)
            sc_infea_011 = ax.scatter(x_i_011, y_i_011, marker='x', c='#009696', alpha=0.3)
            sc_infea_100 = ax.scatter(x_i_100, y_i_100, marker='x', c='#960000', alpha=0.3)
            sc_infea_101 = ax.scatter(x_i_101, y_i_101, marker='x', c='#960096', alpha=0.3)
            sc_infea_110 = ax.scatter(x_i_110, y_i_110, marker='x', c='#969600', alpha=0.3)
            sc_fea = ax.scatter(x_f, y_f, marker='.', c='k', alpha=0.3)
            sc_fea_hof = ax.scatter(x_f_hof, y_f_hof, marker='.', c='orange')
            plt.legend((sc_fea_hof, sc_fea, sc_infea_000, sc_infea_001, sc_infea_010, sc_infea_011, sc_infea_100, sc_infea_101,
                        sc_infea_110), ('Pareto-Menge', 'Zulässig', 'Unzulässig 000', 'Unzulässig 001', 'Unzulässig 010',
                                        'Unzulässig 011', 'Unzulässig 100', 'Unzulässig 101', 'Unzulässig 110'), loc='upper left')
        else:
            sc_fea = ax.scatter(x_f, y_f, marker='.', c='k', alpha=0.3)
            sc_fea_hof = ax.scatter(x_f_hof, y_f_hof, marker='.', c='orange')
            plt.legend((sc_fea_hof, sc_fea), ('Pareto-Menge', 'Zulässig'), loc='upper left')
    ...
    def pareto_front_c_urfd(self, include_infeasible=True, show=True,
                            save=False, formats=('pdf',),
    ...
    def comptime(self, show=True, save=False, formats=('pdf',),
                 basefilename='comptime', xlim=(0, 'max')):
    ...
    def ideas(self, show=True, save=False, formats=('pdf',),
              basefilename='ideas', xlim=(0, 'max')):
        ...
        plt.legend((evals_plot, lu_plot), ('Neu', 'Bekannt'),
                   loc='upper right')
