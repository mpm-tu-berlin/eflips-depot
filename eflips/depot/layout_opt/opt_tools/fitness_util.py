...
def prototype_to_template(dp, simulation_host):
    ...
    nvehicles = sum(count for counts in GC['depot']['vehicle_count'].values()
    ...
    process, errormsg = config.add_process(
        ...
        dur=600,
        ismandatory=False,
        vehicle_filter={'filter_names': ['in_period'],
                        'period': [57600, 20700]},
    ...
    area_prototypes = sorted(dp.areas, key=attrgetter('capacity'),
                             reverse=True)
    ...
    plan, errormsg = config.add_plan(
        'DefaultActivityPlan',
        ID='default',
        locations=['Stauflaeche', 'parking_area_group'] #locations=['Stauflaeche', 'Serviceflaeche', 'parking_area_group']
    )
    if plan is None:
        raise RuntimeError(errormsg)
