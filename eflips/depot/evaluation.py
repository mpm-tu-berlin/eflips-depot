...
class DepotEvaluation:
    ...
    def current_sl(self):
        ...
        *sl_period* in globalConstants defines how far in the
        ...
        trips = [trip for trip in self.depot.pending_departures
        ...
        for area in self.depot.depot_control.departure_areas.stores:
            for vehicle in area.vehicles:
                if vehicle.trip is None:
                    ...
                    if etc is eflips.depot.EstimateValue.COMPLETED \
                            or isinstance(etc, int) and etc <= sl_period:
    ...
    def log_sl(self, *args, **kwargs):
        if globalConstants['depot']['log_sl']:
            ...
            self.sl_logs[self.env.now] = {'trips': trips,
                                          'vehicles': vehicles}
    ...
    def nvehicles_initstore(self, show=True, save=False,
                            basefilename='nvehicles_initstore',
                            formats=('png',)):
        """Vehicle count over time in the depot's init_store. Plot as line."""

        plot_series = self.depot.init_store.logger.get_valList('count',
                                                               self.SIM_TIME)
        ...
        if show or save:
            ...
            ax.plot(plot_series)
            ax.set_title('Depot "' + self.depot.ID
                         + '" number of vehicles in init store')
            plt.xlabel('Time [s]')
    ...
    def nvehicles_initstore_end(self, show=True, save=False,
        ...
        """
        occurrences = [vehicle.vehicle_type.ID for vehicle in
                       self.depot.init_store.items]
        if not occurrences:
    ...
    def nvehicles_used(self, show=True, save=False,
                       basefilename='nvehicles_required', formats=('png',), language=('eng')):
    ...
    def nvehicles_total(self, show=True, save=False,
                        basefilename='nvehicles_total', formats=('png',)):
    ...
    def arrival_soc(self, typs={}, show=True, save=False,
                    basefilename='arrival_soc', formats=('png',), language=('eng')):
        ...
        for arrival_log in self.depot.evaluation.arrival_logs:
            if self.xlim[0] <= arrival_log.arrival_time <= self.xlim[1]:
                ...
                soc_values.loc[arrival_log.arrival_time] = [arrival_log.arrival_time, soc, color]
        ...
        print('Amount of arrivals with negative SoC: %s = %s %% of %s '
              % (count_negative_soc, share_negative_soc, soc_values.shape[0])
              + 'values in total.')
        print('Amount of arrivals with SoC value equal to the reset value %s: '
              '%s = %s %% of %s '
              % (globalConstants['depot']['reset_negative_soc_to'],
                 count_negativity_soc, share_negativity_soc, soc_values.shape[0])
              + 'values in total.')
        ...
        if show or save:
            ...
            soc_values.plot.scatter(x="time", y="soc", s=3,
                                    c=list(soc_values.color), ax=ax)
            if language == 'eng':
                plt.xlabel('Time')
            else:
                plt.xlabel('Zeit')
    ...
    def departure_soc(self, typs={}, show=True, save=False,
                      basefilename='departure_soc', formats=('png',), language=('eng')):
        ...
        if show or save:
            ...
            soc_values.plot.scatter(x="time", y="soc", s=3, c=list(soc_values.color), ax=ax)
            if language == 'eng':
                plt.xlabel('Time')
            else:
                plt.xlabel('Zeit')
    ...
    def get_periods(self, vehicle, vehicle_no, periods):
        ...
        data = {
            'depot general': {'xranges': [], 'yranges': (y, 0.95),
                              'triptexts': []},
            'park': {'xranges': [], 'yranges': (y, 0.75),
                     'slottexts': []}
        }
        ...
        for t in logged_data_areas:
            ...
            if current == previous:     # skip steplog entries where nothing changed for this vehicle
                continue
                ...
                if previous is None:    # enter depot
                    ...
                    if not first_period_found:
                        trip = next((ti for ti in vehicle.finished_trips if
                                     t == ti.ata), None)
                ...
                depot_end_found = True
                park_end_found = True
                data['depot general']['xranges'].append((start_depot,
                                                         t - start_depot))
                data['park']['xranges'].append((start_park,
                                                t - start_park))

                if self.xlim[0] <= start_park <= self.xlim[1]:
                    data['park']['slottexts'].append({
                        'slot_no': previous.ID + str(logged_data_slots[
                                                                 start_park]),
                        'x': start_park,
                        'y': y + 0.5
                        })
        ...
        if not park_end_found:
            data['park']['xranges'].append((
                start_park, self.SIM_TIME - start_park))

            if self.xlim[0] <= start_park <= self.xlim[1]:
                data['park']['slottexts'].append({
                        'slot_no': current.ID + str(logged_data_slots[
                                                                start_park]),
                        'x': start_park,
                        'y': y + 0.5
                        })
    ...
    def vehicle_periods(self, periods=None, show=True, save=False,
                        basefilename='vehicle_periods', formats=('pdf',),
                        show_vehicle_count=True, vehicle_count_color='#1f77b4',
                        show_total_power=True, total_power_color='#d62728',
                        show_annotates=True, vehicle_ID='all',
                        smart_charging=None, smart_charging_color = "forestgreen"):
        ...
        for vehicle in self.vehicle_generator.items:
            if vehicle_ID == 'all' or vehicle_ID in vehicle.ID:
                ...
                vehicle_no += 1
        if not vehicledata:
            print("No data for vehicles found. Possibly argument 'vehicle_ID' "
                  "doesn't match any ID.")
            return

        if isinstance(smart_charging,eflips.depot.smart_charging.SmartCharging):
            for vehicle_ID, data in smart_charging.export_for_vehicle_periods_smart().items():
        ...
        if show or save:
            ...
            for ID in vehicledata:
                ...
                for periodID in vehicledata[ID]['plotdata']:
                    print('\t', periodID, ': ')
                    print('\t\txranges: ', vehicledata[ID]['plotdata'][
                        periodID]['xranges'])
                    print('\t\tyranges: ', vehicledata[ID]['plotdata'][
                        periodID]['yranges'])
            ...
            ax1.set_yticks([(vehicle_no + 1) + 0.5 for vehicle_no in range(len(
                vehicledata))])
            # ax1.set_yticklabels(vehicledata.keys(), fontsize=ticklabelsize)
            ax1.set_yticklabels([i + 1 if (i + 1) % 10 == 0 else "" for i in range(len(vehicledata))],
            ...
            marks = [mpatch.Rectangle((0, 0), 1, 1, fc=clr)
                     for clr in periods.values()]
            names = list(periods.keys())

            # Additional plot in same window: vehicle count
            # Sum up the counts of all areas in the depot
            if show_vehicle_count:
                y2 = np.zeros(self.SIM_TIME,
                              dtype=np.int32)
                for area in self.depot.list_areas:
                    nv = area.logger.get_valList('count',
                ...
                ax2.set_yticks([(vehicle_no) + 0.5 for vehicle_no in range(len(
                    vehicledata) + 1)])
                ax2.set_yticklabels(range(len(vehicledata) + 1),
                                    fontsize=ticklabelsize)
                ax2.set_zorder(-2)
            ...
            if show_total_power:
                ...
                max_possible_power = sum(
                    [ci.max_power for ci in self.depot.resources.values()
            ...
            if show_annotates:
                ...
                for vID in vehicledata:
                    for tripInfo in vehicledata[vID]['plotdata']['depot general']['triptexts']:
                        xoffset = xoffset_arr if tripInfo['as_arrival'] else xoffset_dep
                        color_txt = delay_color if tripInfo['delay'] != 0 else 'black'
                        text = ax4.text(
                            tripInfo['x'] + xoffset, tripInfo['y'],
                            tripInfo['line_name'], fontproperties=font,
                            **alignment, color=color_txt, rotation=rotation)
                        ...
                        if tripInfo['delay'] != 0:
                            ...
                            ax4.text(tripInfo['x'] + xoffset, tripInfo['y']-0.7,
                                     str(int(tripInfo['delay']/60)) + " min", fontproperties=font,
                                     **alignment, color=color_txt,
                                     rotation=rotation)
                ...
                for vID in vehicledata:
                    for slotInfo in vehicledata[vID]['plotdata']['park']['slottexts']:
                        text = ax4.text(slotInfo['x'] + xoffset, slotInfo['y'] + yoffset,
                                 slotInfo['slot_no'], fontproperties=font,
                                  color=color_txt, rotation=rotation)
                        x_pos = text.get_position()[0]
                        # set texts outside xlim to invisible
                        if not(self.xlim[0] < x_pos < self.xlim[1]):
                            text.set_visible(False)
    ...
    def idle_time_dist(self, charge_IDs=('charge',), vehicle_types='all',
                       show=True, save=False, basefilename='idle_time_dist',
                       formats=('png',), bins=100, color='#1f77b4'):
        ...
        for vehicleID in vehicledata:
            if vehicle_types == 'all' or self.vehicle_generator.select(
                    vehicleID).vehicle_type.ID in vehicle_types:
                for charge_ID in charge_IDs:
        ...
        print('\tmin: ', int(it_min * 60), ' s; = ', "%.2f" % it_min,
              ' min')
        it_max = idle_times_array.max()
        self.results['idle_time'][vt_joined]['max'] = it_max
        print('\tmax: ', int(it_max * 60), ' s; = ', "%.2f" % it_max,
              ' min')
        it_mean = np.mean(idle_times_array)
        self.results['idle_time'][vt_joined]['average'] = it_mean
        print('\taverage: ', int(it_mean * 60), ' s; = ', "%.2f"
              % it_mean, ' min')
        it_median = np.median(idle_times_array)
        print('\tmedian: ', int(it_median * 60), ' s; = ',
              "%.2f" % it_median, ' min')
        print('\toccurrences of 0: ', np.count_nonzero(
            idle_times_array == 0))
        print('\toccurrences of <= 15 min: ', np.count_nonzero(
            idle_times_array <= 15))
        ...
        if show or save:
            ...
            plt.title('Distribution of idle time (parking time after '
    ...
    def total_power(self, show=True, save=False, basefilename='total_power',
                    formats=('png',)):
    ...
    def ci_power(self, cis=[1, 1], show=True, save=False, basefilename='total_power',
                    formats=('png',), language=('eng')):
        ...
        power_logs_cis = np.empty_like(discrete2continuous_logs(self.depot.resources['ci_' + str(cis[0])].power_logs_ci, first=0))
        ci_counter = 0

        for ci_no in range(cis[0], cis[1]+1):
            power_logs_ci = discrete2continuous_logs(self.depot.resources['ci_' + str(ci_no)].power_logs_ci, first=0)
            # print(power_logs_ci)
    ...
    def sl_single(self, vehicle_type_ID, show=True, save=False,
                  basefilename='stress_level', formats=('pdf',)):
        ...
        vt_obj = next(
            (vt for vt in self.gc['depot']['vehicle_types_obj']
             if vt.ID == vehicle_type_ID), None)
        if vt_obj is None:
        ...
        if show or save:
            ...
            plot_title = 'Stress level for vehicle type(s) %s for a period of %s hours' \
    ...
    def sl_all(self, show=True, save=False, basefilename='stress_level',
               formats=('pdf',)):
        ...
        self.results['sl']['categories'] = globalConstants['depot']['vehicle_type_categories'][::-1]

        sl_values = []
        for category in self.results['sl']['categories']:
            sl = discrete2continuous_logs(self.depot.evaluation.sl_logs,
                                           first=0, subkeys=['sl', category])
            sl_values.append(sl)
        ...
        if show or save:
            # Print
            period = globalConstants['depot']['sl_period']
            print('All current dispatch safety values after the first day (later than %s) for a period of %s hours:'
            ...
            plot_title = 'Current dispatch safety for all vehicle types or vehicle type groups for a period of %s hours' \
                         % (period / 3600)
            ax.set_title(plot_title)
            plt.xlim(*self.xlim)
            to_dateaxis(ax)
        ...
        for area in self.depot.list_areas:
            if area.issink:
                if 'count_rfd_unblocked' not in area.logger.loggedData:
                    print("Plot 'count_rfd_unblocked' requires logging of "
                          "'count_rfd_unblocked' at areas.")
                    return

            yi = area.logger.get_valList('count_rfd_unblocked',
                                         SIM_TIME=self.SIM_TIME)
    ...
    def count_rfd_unblocked_total(self, show=True, save=False,
    ...
    def nvehicles_area(self, area_ID, show=True, save=False,
                       basefilename='nvehicles_area', formats=('png',), language='eng'):
    ...
    def nvehicles_group(self, group_ID, show=True, save=False,
                       basefilename='nvehicles_group', formats=('png',), language='eng'):
        """Plot vehicle count over time on the area with *area_ID*."""
    ...
    def count_rfd_unblocked(self, area_ID, show=True, save=False,
        ...
        if 'count_rfd_unblocked' not in area.logger.loggedData:
            print("Plot 'count_rfd_unblocked' requires logging of "
                  "'count_rfd_unblocked' at areas.")
            return
        plot_series = area.logger.get_valList('count_rfd_unblocked',
                                              SIM_TIME=self.SIM_TIME)
        plot_series = to_prev_values(plot_series)
    ...
    def nvehicles_generated(self, show=True, save=False,
    ...
    def std_line(self, show=True, save=False,  basefilename='std_line',
                 formats=('png',)):
    ...
    def tripattr_scatter(self, attr, plot_title, basefilename, show=True,
                         save=False, formats=('png',)):
    ...
    def std_scatter(self, show=True, save=False, basefilename='std_scatter',
                    formats=('png',)):
        """Plot SimpleTrip.std as scatter."""
        self.tripattr_scatter('std', 'Scheduled departure times of trips',
                              basefilename, show, save, formats)

    def sta_scatter(self, show=True, save=False, basefilename='sta_scatter',
                    formats=('png',)):
        """Plot SimpleTrip.sta as scatter."""
        self.tripattr_scatter('sta', 'Scheduled arrival times of trips',
                              basefilename, show, save, formats)

    def ata_scatter(self, show=True, save=False, basefilename='ata_scatter',
                    formats=('png',)):
        """Plot SimpleTrip.ata as scatter."""
        self.tripattr_scatter('ata', 'Actual arrival times of trips',
                              basefilename, show, save, formats)

    def lead_time_match_scatter(self, show=True, save=False,
    ...
    def lead_time_match_dist(self, show=True, save=False,
        ...
        print('Successful disposition at lead time: ' + str(tsm_lead/tsm*100) + ' %')
    ...
    def trip_distances_scatter(self, show=True, save=False,
    ...
    def trip_distances_dist_bins(
            self, bins=15, distance_limit=150, show=True, save=False,
            basefilename='trip_distance_distribution', formats=('png',)):
        ...
        for s_tr in plot_series:
            if s_tr >= distance_limit:
                n_crit_trips += 1
        print('No of trips with distance higher than or equal to %s: %s out '
              'of %s, which equals %s %%'
              % (distance_limit, n_crit_trips, n_trips,
                 n_crit_trips / n_trips * 100))
    ...
    def trip_distances_dist(
            ...
            basefilename='trip_distance_distribution', formats=('png',), language='eng'):
        """Plot trip (without repetition) distance in a histogram.
        ...
        for vt in vehicle_types:
            ...
            distances = [trip.distance for trip in self.timetable.trips if trip.vehicle_types_joinedstr == vt]
            distances.sort()
    ...
    def target_vehicle_types_dist(self, show=True, save=False,
        basefilename='target_vehicle_types_distribution', formats=('png',)):
        """Plot the distribution of vehicle_types of trips (without
        repetition) in a histogram.
        """
        occurrences = [trip.vehicle_types_joinedstr
    ...
    def departure_delay_vt(self, vehicle_types=None, show=True, save=False,
                           basefilename='departure_delay_vt', formats=('png',)):
        ...
        for vt in delaydata:
            ...
            if delaydata[vt]['total_trip_count'] > 0:
                ...
                delaydata[vt]['share_delayed'] = delaydata[vt]['count'] / delaydata[vt]['total_trip_count'] * 100
                # Mean of delays including not delayed
                delaydata[vt]['mean_all'] = delaydata[vt]['sum'] / delaydata[vt]['total_trip_count']
            else:
        ...
        share_delayed_all = count_delayed_all / count_all * 100 \
            if count_all > 0 else 0
        average_all = sum_all / count_all
        average_all_delayed = sum_all / count_delayed_all \
            if count_delayed_all > 0 else 0

        for vt in delaydata:
            print('\nVehicle type %s:' % vt)
            print('\ttotal number of finished trips: %d'
                  % delaydata[vt]['total_trip_count'])
            print('\tnumber delayed trips: %d (%.2f %%)'
                  % (delaydata[vt]['count'],
                     round(delaydata[vt]['share_delayed'], 2)))
            print('\tmax delay: %d min' % (delaydata[vt]['max']/60))
            print('\tdelay sum: %d min' % (delaydata[vt]['sum']/60))
            print('\tmean delay including trips on time: %d min'
                  % (delaydata[vt]['mean_all']/60))
            print('\tmean delay only for delayed trips: %d min'
                  % (delaydata[vt]['mean']/60))

        print('\nOverall:')
        print('\ttotal number of finished trips: %d' % count_all)
        print('\tnumber of delayed trips: %d (%.2f %%)'
        ...
        print('\tmean delay only for delayed trips: %d min' % (average_all_delayed/60))
        ...
        if show or save:
            ...
            for vt in delaydata:
                ...
                plot_series = [None if t == 0 else t for t in
                               delaydata[vt]['all']]
    ...
    def total_delay(self):
        ...
        for trip in self.timetable.trips_issued:
            if trip.std < self.timetable.env.now and (trip.atd is None or trip.atd > trip.std):
                self.results['total_delay'] += trip.delay_departure

    # Evaluation and plots specific for vehicles
    def area_history(self, vehicle_ID, show=True, save=False,
                     basefilename='area_history', formats=('png',)):
        ...
        if vehicle is None:
            raise ValueError("Vehicle with ID '%s' couldn't be found"
                             % vehicle_ID)
        ...
        plotdata = OrderedDict(sorted(plotdata.items(),
                                      key=lambda pair: areas_chron[pair[0]]))
        ...
        if show or save:
            ...
            if show:
                fig.show()
            if save:
                filename = self.path_results + 'vehicle_' + vehicle_ID + '_' \
                           + basefilename
                savefig(fig, filename, formats)
            if not show:
                plt.close(fig)

    def vehicle_type_dist(self, show=True, save=False,
                          basefilename='vehicle_type_distribution',
                          formats=('png',)):
        """Plot the distribution of vehicle types in a histogram."""
        occurrences = [vehicle.vehicle_type.ID for vehicle in self.vehicle_generator.items]
        vehicle_types = list(set(occurrences))
        typemap = {}
        for vehicletypeNo, vehicletype in enumerate(vehicle_types):
            typemap[vehicletype] = vehicletypeNo
    ...
    def processhistory_single_vehicle_separate(
            self, vehicle_ID, show=True, save=False,
        ...
        if show or save:
            ...
            ax.grid(True)
            to_dateaxis(ax)
            plt.title('Vehicle "%s" (type %s) process history'
                      % (vehicle_ID, vehicle.vehicle_type.ID))
    ...
    def process_intervals(
            self, process_ID, crit_value=86400*2, bins=15, show=True,
            save=False, basefilename='process_intervals', formats=('png',)):
        ...
        if process_ID not in self.depot.processes:
            raise ValueError("Process with ID '%s' not found in depot '%s'"
        ...
        print('\n Summary of all intervals: ',
              'min: ', intervals_all['min'],
              ', max: ', intervals_all['max'],
        ...
        if show or save:
            ...
            plt.xlabel('time [s]')
            plt.title("Distribution of intervals of process '%s' for all "
                      "vehicles" % process_ID)
            # plt.xlim(left=0)
            plt.ylim(bottom=0)
            plt.grid()
    ...
    def battery_level(self, vehicle_ID, show=True, save=False,
                      basefilename='battery_level', formats=('png',)):
        ...
        for current in vehicle.battery_logs:
            ...
            y[previous.t: current.t] = np.linspace(previous.energy,
                                                   current.energy,
        ...
        if show or save:
            ...
            plt.grid(True)
            plt.title('Vehicle %s (type %s) battery level history' % (
                vehicle_ID, vehicle.vehicle_type.ID))
            to_dateaxis(ax)
    ...
    def vehicle_power(self, vehicle_ID, show=True, save=False,
                      basefilename='vehicle_power', formats=('png',)):
    ...
    def usage_history(self, resource_ID, show=True, save=False,
                      basefilename='usage_history', formats=('png',)):
        ...
        for t in resource.logger.loggedData['user_count']:
            ...
            if users:
                for user in users:
                    ...
                    user_counts[user][t] = resource.logger.loggedData[
                        'user_count'][t][user]
    ...
    def congestion_calulation(self):
        ...
        for vehicle in self.vehicle_generator.items:
            ...
            for key, item in vehicle.logger.loggedData["area_waiting_time"].items():
                inter_interval = intersection(self.xlim, [key - item["waiting_time"], key])
            ...
            for trip in vehicle.finished_trips:  # Time in depot
                if pre_ata is not None:
                    ...
                    if isinstance(inter_interval, list):
                        total_dwelltime += (inter_interval[1] - inter_interval[0]) / 60 / 60
                pre_ata = trip.ata

        return total_congestion, congestion_area_caused, \
               congestion_area_stands, congestion_vehicle, total_dwelltime

    def congestion(self, show=True, save=False,
        ...
        (total_congestion, congestion_area_caused, congestion_area_stands, congestion_vehicle,total_dwelltime) = self.congestion_calulation()

        print('\nTotal congestion in time intervall: %.2f h' % total_congestion)
        print('Percentage of total congestion time on total dwelltime in time intervall %.2f %%' % ((total_congestion/total_dwelltime)*100))
        ...
        if show or save:
            ...
            new_index = [i + 1 if (i + 1) % 10 == 0 else "" for i in range(len(congestion_vehicle))]
            ...
            for time, slot in vehicle.logger.loggedData["dwd.current_slot"].items():
                ...
                if pre_log["slot"] is not None and pre_log["area"] is not None:
                    ...
                    if isinstance(inter_interval, list):
                        ...
                        areas_and_slots[pre_log["area"]].loc[pre_log["slot"]] += time_delta

                if isinstance(vehicle.logger.loggedData["dwd.current_area"][time], eflips.depot.depot.BaseArea):
                    pre_log["area"] = vehicle.logger.loggedData["dwd.current_area"][time].ID
                else:
                    pre_log["area"] = vehicle.logger.loggedData["dwd.current_area"][time]
                pre_log["slot"] = slot
                pre_log["time"] = time
    ...
    def occupancy_rate(self, basefilename="occupancy_rate.xlsx"):
        ...
        metadata = ["time", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'templatename',
                    self.configurator.templatename, 'depot ID', self.depot.ID]
        ...
        for key, series in areas_and_slots.items():
            ...
            percentage_sheet.write_column(row=4, col=i, data=column, cell_format=per_format)
        ...
        absolute_sheet.write_row(row=2, col=0, data=["cumulated sum (all slots in depot)", absolute_depot])
        percentage_sheet.write_row(row=2, col=0, data=["average occupation (all slots in depot)", per_depot/areas_and_slots.__len__()], cell_format=per_format)

        wb.close()

    def blocked_slots(self, show=True, save=False,
                      basefilename='blocked_slots', formats=('png',)):
        """
        ...
        for area in self.depot.areas.values():
            if isinstance(area, eflips.depot.LineArea):
                ...
                nv = area.logger.get_valList('vacant_blocked',
                                             SIM_TIME=self.SIM_TIME)
                nv = to_prev_values(nv)
                nv = np.array(nv)
        ...
        print("Max. number of used and blocked slots (LineArea): " + str(count_together.max()))
        print("Max. number of blocked slots(LineArea): " + str(count_blocked_slots.max()))
    ...
    def pssmart2_ratings(self, show=True, save=False,
                         basefilename='pssmart2_ratings', formats=('png',)):
            ...
            for time, values in group.pssmart2_logs.items():
                for value in values:
                    if value is not None:
                        ...
                        y_none.append(0)
        if not x and not y:
            raise ValueError('park_ratings requires data in ParkingAreaGroup.pssmart2_logs.')
        ...
        if show or save:
            ...
            print( 'Number of invalid values (no alternatives at the time of rating): %d' % len(x_none))
...
def to_dateaxis(ax):
    ...
    ax.xaxis.set_minor_locator(ticker.AutoMinorLocator(
        minor_intervals_per_major_tick))
    ...
    ax.xaxis.grid(which='minor', color='lightgrey', linestyle='--',
                  dashes=(5, 5))
...
class Report:
    ...
    def get_depot_logs(self):
        ...
        # Area count
        for area in itertools.chain(self.depotsim.depot.list_areas,
                                    [self.depotsim.depot.init_store]):
        ...
        for t, subd in enumerate(depot_logs):
            ...
            for k in all_keys:
                if k not in subd:
                    ...
                    back = 1
                    while not found:
                        if depot_logs[t - back] is not None and k in depot_logs[t - back]:
    ...
    def append_tripdata(self, wb):
        ...
        for trip in self.depotsim.depot.timetable.trips_issued:
            ...
            row = [
                ...
                abs_time(trip.actual_duration) if trip.actual_duration is not None else None,
                ...
                abs_time(trip.delay_departure) if trip.delay_departure is not None else None
            ]
...
class DepotAnalysis:
    ...
    def defaultname(self):
        ...
        return self.configurator.templatename + '_export_' \
    ...
    def export_logs(self, filename):
        ...
        if filename:
            try:
                ...
                # meta
                meta_dateTime = ["time", datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S")]
                ws.append(meta_dateTime)

                ws.append(['templatename',
                ...
                # headers
                headers = ["event ID", "sim time", "vehicle ID",
                           "vehicle type", "battery level", "battery capacity",
                           "area ID", "power", "event name",
                           "event relevance CM", "overdue trips",
                           "depot total count"]
                ...
                for event_ID, log in enumerate(self.logs):
                    ...
                    relevance_cm = log.area is not None \
                        and log.area.issink and (
                            log.action == 'ACTION_CHARGE_START'
                            or log.action == 'ACTION_CHARGE_FULL'
                            or log.action == 'ACTION_POP_DEPART')

                    row = [event_ID, log.simTime, log.vehicle.ID,
                           log.vehicle.vehicle_type.ID, log.battery_level,
                           log.vehicle.battery.energy_real, current_area_ID,
                           power, log.action, relevance_cm,
                           log.overdue_trips, log.depot_count_total]
                ...
                if hasattr(self.depot, 'view'):
                    ...
                    filename_full = globalConstants['depot']['path_results'] \
                                    + filename + '.xlsx'
                    wb.save(filename_full)
                    print('Vehicle event data and trip data exported to %s'
                          % filename_full)
    ...
    def export_logs_smart_charging(self):
        ...
        if not self.logs:
            raise RuntimeError(
                ...
                "simulation with the option switched on.")
        # headers
        headers = [ "sim_time", "vehicle_ID",
                   "vehicle_type", "battery_level", "battery_capacity",
                    "power", "event_name" ]
        ...
        for event_ID, log in enumerate(self.logs):
            ...
            relevance_cm = log.area is not None and (
                    ...
                    or log.action == 'ACTION_POP_DEPART')
            if relevance_cm:
                row = [log.simTime, log.vehicle.ID,
                       log.vehicle.vehicle_type.ID, log.battery_level,
    ...
    def append_tripdata(self, wb):
        ...
        # headers
        headers = ['ID', 'origin', 'destination',
                   'vehicle_types', 'std', 'sta',
                   'distance', 'atd', 'ata',
                   'eta', 'vehicle.ID',
                   'vehicle.vehicleType', 'reserved_for_init',
                   'vehicle_from', 'overdue']

        ws.append(headers)

        # Add sheet with summarizing infos
        ws2 = wb.create_sheet('TripChecks')
        vehicle_types = list(globalConstants['depot'][
                                 'vehicle_types'].keys())
        overdueChecks = []
        ...
        for trip in self.depot.timetable.trips_issued:
            ...
            pos3 = trip.vehicle_from if hasattr(trip,
                                                'vehicle_from') else None
            pos4 = trip.delayed_departure

            row = [trip.ID, trip.origin.ID, trip.destination.ID,
                   trip.vehicle_types_joinedstr, trip.std,
                   trip.sta, trip.distance, trip.atd,
                   trip.ata, trip.eta, pos1,
                   pos2, trip.reserved_for_init, pos3, pos4]
        ...
        # Summarize
        headers2 = ['any overdue', 'EN overdues', 'GN overdues',
                    'DL overdues', 'EN unique', 'GN unique',
                    'DL unique'] + tck

        addData = [overdueCheck] \
