...
class SmartCharging:
    ...
    def __init__(self, data,start_date,price_data_path, power_limit_grid):
        ...
        if isinstance(data, eflips.depot.evaluation.DepotEvaluation):#for data straight out of eflips
            self.schedule_method(data)
        if isinstance(data, pd.core.frame.DataFrame):#for schedule from externeal programms. Watch that the format is correct.
            self.schedule = data
        ...
        try:
            ...
            print("The start date has the wrong format. I use the 2019-06-03 as the start date.")
            self.start_date = datetime.date(2019, 6, 3)

        power_item = eflips.depot.PowerFrame(self.max_sim_time, self.start_date, price_data_path)
    ...
    def schedule_method(self,ev):
        ...
        columns = ["charging_power", "energy_demand_real","energy_demand", "time_arrival", "time_depart",
                   "time_charging_flex", "charging_efficiency"]  # units[kW, kWs,kWs, s, s, s,1] energy_demand_calc is the box integral, which is used for the calculation
        ...
        precondition_columns = ["charging_power","time_start","time_ends","charging_efficiency"]# units[kW,  s, s, 1]
        ...
        for vehicle in ev.vehicle_generator.items:
            ...
            for process_time, process_list_item in vehicle.logger.loggedData["dwd.active_processes_copy"].items():

                if isinstance(pre_process_item_charge, eflips.depot.processes.ChargeAbstract):

                    row={}
                    row["time_arrival"] = pre_process_item_charge.starts[0]
                    row["time_depart"] = process_time #Depart time of the process before is the estart time of the next process

                    charging_duration = self.calc_charging_time(pre_process_item_charge.starts, pre_process_item_charge.ends)
                    row["time_charging_flex"] = row["time_depart"] - row["time_arrival"] - charging_duration

                    row["energy_demand_real"] = pre_process_item_charge.energy *3600 #kWh ->kWs
                    ...
                    if row["energy_demand_real"] > row["energy_demand"]: #Makes sure the that the numerical error gets deleted
                    ...
                    if all_keys_exist: # safes the row in the dataframe
                        ...
                        if row["energy_demand"]/ row["energy_demand_real"] > 2:
                            print("Bus "+vehicle.ID+" has deviation of over 50 percent between energy_demand_real and energy_demand. You might wanna check this.")
                    else:
                        row = pd.Series(row)
                ...
                for process_item in process_list_item:
                    ...
                    if isinstance(process_item, eflips.depot.processes.Precondition) and process_item is not pre_precon_process:
                        # Filter and Store the correct processes
                        row = {"charging_power": process_item.power, "time_start": process_item.starts[0]}
                        ...
                        if process_item.ends: # Checks if Precon process is complete
                            ...
                            row = pd.Series(row)
                            self.precondition_schedule.loc[vehicle.ID + "_" + str(i_precon)] = row
        ...
        for ID_bus, row_bus in schedule.iterrows():
            ...
            for ID_power, row_power in self.power.iterrows():
                if row_bus.energy_demand  <= 0:
                    break
                power_rest = row_power.limit_grid - row_power.used_power_smart #determines the left power
                interval = self.intersection([row_power.start_time, row_power.end_time],
                                             [row_bus.time_arrival, row_bus.time_depart])
                if isinstance(interval, list) and power_rest > 0:
                    self.energy_calculation_smart(interval[0], interval[1], row_bus, row_power,
                                                       ID_power, power_rest, ID_bus)
            ...
            if energy_delta > 0:
                self.compensate_to_energy_real(energy_delta,ID_bus,row_bus,self.charging_log,"used_power_smart")


        return True

    def energy_calculation_smart(self,start_time, end_time, row_bus, row_power, ID_power, power_rest,ID_bus):
        """
        ...
        :return:
        """
        factor_power_used = (end_time-start_time)/(row_power.end_time - row_power.start_time) #a factor which scales the power, if a bus intervall is not complet in a power intervall
        ...
        if power_rest >= row_bus.charging_power / row_bus.charging_efficiency:
            if row_bus.energy_demand >= (end_time - start_time) * row_bus.charging_power:
                ...
                power = (row_bus.charging_power / row_bus.charging_efficiency)* factor_power_used
                self.power.at[ID_power, "used_power_smart"] += power
                row = [start_time,end_time,power*row_bus.charging_efficiency, row_power.price]
                self.charging_log[ID_bus].loc[int(start_time)] = row
            else:
                power = row_bus.energy_demand / ((end_time - start_time)*row_bus.charging_efficiency)* factor_power_used
                self.power.at[ID_power, "used_power_smart"] += power
                row = [start_time, end_time, power*row_bus.charging_efficiency, row_power.price]
                self.charging_log[ID_bus].loc[int(start_time)] = row
            ...
            if row_bus.energy_demand >= (end_time - start_time) * power_rest * row_bus.charging_efficiency:
                energy = (end_time - start_time) * power_rest * row_bus.charging_efficiency
                row_bus.energy_demand -= energy
                #self.energy_log.at[row_power.start_time, ID_bus] = energy
    ...
    def smart_charging_algorithm_even(self): # Outdated, not maintained
        ...
        for ID_bus, row_bus in self.schedule.iterrows():
            constant_power = row_bus.energy_demand/(row_bus.time_depart - row_bus.time_arrival)

            for ID_power, row_power in self.power.iterrows():

                interval = self.intersection([row_power.start_time,row_power.end_time],[row_bus.time_arrival,row_bus.time_depart])
                if isinstance(interval,list):
                    self.energy_calculation_smart_even(interval[0], interval[1], row_bus, row_power,
                                                   ID_power, ID_bus, constant_power)
        ...
        for ID_power, row_power in self.power.iterrows():
            if row_power.used_power_smart_even > 0:
                scaling_quotients = row_power.limit_grid/row_power.used_power_smart_even
        ...
        if any(self.power.used_power_smart_even > self.power.limit_grid*1.01): #*1.01 because of the inaccuracy of the numeric calculations, in the calculations before
            print("Smart even failed")
            return False
        else:
            return True

    def energy_calculation_smart_even(self,start_time, end_time, row_bus, row_power, ID_power, ID_bus, constant_power): # Outdated, not maintained

        factor_power_used = (end_time - start_time) / (
                    row_power.end_time - row_power.start_time)  # a factor which scales the power, if a bus intervall is not complet in a power intervall
    ...
    def smart_even_upscalinig(self,scaling_quotients,ID_power): # Outdated, not maintained
        ...
        for ID_bus, value in self.charging_log_even.items():
            for start_time, row in value.iterrows():
                if ID_power == start_time:
                    #Some checks
                    if row.power * scaling_quotients > self.schedule.at[ID_bus,"charging_power"]:
                        power_delta = self.schedule.at[ID_bus,"charging_power"] - row.power
                        self.power.at[ID_power, "used_power_smart_even"] += power_delta
                        self.substract_power_most_expensive(power_delta,ID_bus,ID_power)
                    else:
                        power_delta = (row.power * scaling_quotients) - row.power
                        self.power.at[ID_power, "used_power_smart_even"] += power_delta
                        self.substract_power_most_expensive(power_delta, ID_bus, ID_power)

    def substract_power_most_expensive(self,power,ID_bus,ID_power): # Outdated, not maintained
        """Substracts the power from an other power intervall."""
        self.charging_log_even[ID_bus] = self.charging_log_even[ID_bus].sort_values(by=["price"], ascending=False)
        for start_time, row in self.charging_log_even[ID_bus].iterrows():
            if start_time != ID_power:
                if row.power >= power:
                    ...
                    self.power.at[start_time, "used_power_smart_even"] -= self.charging_log_even[ID_bus].at[start_time,"power"]
                    self.charging_log_even[ID_bus].at[start_time, "power"] = 0

    def smart_even_downscalinig(self,scaling_quotients,ID_power): # Outdated, not maintained
    ...
    def add_power_less_expensive(self, power, ID_bus, ID_power): # Outdated, not maintained
        # Adds the power to an other intervall.
        self.charging_log_even[ID_bus] = self.charging_log_even[ID_bus].sort_values(by=["price"])
        for start_time, row in self.charging_log_even[ID_bus].iterrows():
            if start_time != ID_power:
                potential_power = min([self.schedule.at[ID_bus,"charging_power"],self.power.at[start_time,"limit_grid"] - self.power.at[start_time,"used_power_smart_even"]]) #max power of charging infrastructur of a singele bus, left power until max of the grid
                if potential_power >= power:
                    self.charging_log_even[ID_bus].at[start_time,"power"] += power
                    self.power.at[start_time,"used_power_smart_even"] += power
                    break
                else:
                    self.charging_log_even[ID_bus].at[start_time, "power"] += potential_power
                    self.power.at[start_time, "used_power_smart_even"] += potential_power
        ...
        for ID_bus, row_bus in schedule.iterrows():
            ...
            for ID_power, row_power in self.power.iterrows():
                interval = self.intersection([row_power.start_time, row_power.end_time],
                                             [row_bus.time_arrival, row_bus.time_depart])
                if isinstance(interval,list):
                    self.energy_calculation_immediately(interval[0], interval[1], row_bus, row_power,ID_power,ID_bus)
            ...
            # Make adjustments so the charging energy fit the real energy
            if energy_delta > 0:
                self.compensate_to_energy_real(energy_delta, ID_bus, row_bus, self.charging_log_imm,"used_power_immediately")
    ...
    def energy_calculation_immediately(self,start_time, end_time, row_bus, row_power, ID_power,ID_bus):
        """
        Helper
        :param start_time:
        :param end_time:
        ...
        if row_bus.energy_demand >= (end_time - start_time) * row_bus.charging_power:
            ...
            self.power.at[ID_power, "used_power_immediately"] += (row_bus.charging_power / row_bus.charging_efficiency) * factor_power_used
            row = [start_time, end_time, row_bus.charging_power, row_power.price]
            self.charging_log_imm[ID_bus].loc[int(start_time)] = row
        else:
            power_at_grid = (row_bus.energy_demand / ((end_time - start_time) * row_bus.charging_efficiency)) * factor_power_used
            self.power.at[ID_power, "used_power_immediately"] += power_at_grid
            row_bus.energy_demand = 0
            row = [start_time, end_time, power_at_grid*row_bus.charging_efficiency, row_power.price]
            self.charging_log_imm[ID_bus].loc[int(start_time)] = row
    ...
    def compensate_to_energy_real(self, energy_delta, ID_bus, row_bus, charging_log,type_of_power):
        """
        ...
        charging_log[ID_bus] = charging_log[ID_bus].sort_values(by="start", ascending=False)
        for start_time, charg_log in charging_log[ID_bus].iterrows():
            ...
            if energy_delta >= energy_slot:
                ...
                self.power.at[power_time, type_of_power] -= charg_log.power / row_bus.charging_efficiency
                ...
                self.power.at[power_time, type_of_power] -= (charg_log.power - power_at_bus) /row_bus.charging_efficiency
    ...
    def precondition(self):
        ...
        for ID_bus, row_bus in self.precondition_schedule.iterrows():
            for ID_power, row_power in self.power.iterrows():
                ...
                if isinstance(interval, list):
                    factor_power_used = (interval[1] - interval[0]) / (row_power.end_time - row_power.start_time)
                    power = (row_bus.charging_power / row_bus.charging_efficiency) * factor_power_used
    ...
    def plot_results(self, language='eng'):
        ...
        ax1.set_title('Charging power (all buses) and power price, Date: ' + str(self.start_date))
        ...
        # plot precondition
        if language == 'eng':
            precondition, = ax1.plot(x, precondition, color="k", linestyle='--', linewidth=1, label="precondition")
        else:
            precondition, = ax1.plot(x, precondition, color="k", linestyle='--', linewidth=1, label="Vorkonditionierung")

        # plot power limit grid
        if language == 'eng':
            power_limit_grid, = ax1.plot(x, power_limit_grid, color="k", linewidth=1, label="power limit grid")
        else:
            power_limit_grid, = ax1.plot(x, power_limit_grid, color="k", linewidth=1, label="Limit Gesamtladeleistung")
        ...
        ax2 = ax1.twinx()
        if language == 'eng':
            spot_price, = ax2.plot(x, price, color="r", linewidth=1, label="spot price")
        else:
            spot_price, = ax2.plot(x, price, color="r", linewidth=1, label="Preis Spotmarkt")
        if language == 'eng':
        ...
        if language == 'eng':
            lightgrey_patch = mpatches.Patch(color='grey', label='non-controlled charging', alpha=0.3)
        else:
            lightgrey_patch = mpatches.Patch(color='grey', label='Lademanagement: Nicht gesteuert', alpha=0.3)
        if language == 'eng':
            grey_patch = mpatches.Patch(color='grey', label='smart charging', alpha=0.7)
        else:
            grey_patch = mpatches.Patch(color='grey', label='Lademanagement: Smart', alpha=0.7)

        plt.legend(handles=[lightgrey_patch, grey_patch, power_limit_grid, precondition, spot_price], mode="expand", ncol=2)

        plt.show()
        fig.savefig(globalConstants['depot']['path_results'] + 'charging_profile_non-controlled_vs_smart.pdf')
    ...
    def results(self):
        ...
        if "used_power_immediately" in self.power.columns:
            ...
            energy_ic = round((self.power.used_power_immediately * (self.power.end_time - self.power.start_time) / 3600).sum())
            results.at["Energy (kWh)", "Imm. Charging"] = energy_ic

            energy_ic_interval = round((power_sorted_interval.used_power_immediately * (power_sorted_interval.end_time - power_sorted_interval.start_time) / 3600).sum())
            ...
            energy_ic_cost_fix = round(((self.power.used_power_immediately * (self.power.end_time - self.power.start_time) / 3600) * 0.0433).sum(), 2)
            results.at["Energy Costs (€)", "Imm. Charging"] = energy_ic_cost_fix

            energy_ic_cost_fix_interval = round(((power_sorted_interval.used_power_immediately * (power_sorted_interval.end_time - power_sorted_interval.start_time) / 3600) * 0.0433).sum(), 2)
            results.at["Energy Costs Interval (€)", "Imm. Charging"] = energy_ic_cost_fix_interval

            grid_ic_cost = round(max_power_ic * 66.82 / 365 * self.max_sim_time / (24 * 60 * 60) + energy_ic * 0.0098, 2)
            results.at["Grid Costs (€)", "Imm. Charging"] = grid_ic_cost

            grid_ic_cost_interval = round(max_power_ic * 66.82 / 365 + energy_ic_interval * 0.0098, 2)
            results.at["Grid Costs Interval (€)", "Imm. Charging"] = grid_ic_cost_interval

            usage_per_year_ic = round(energy_ic / max_power_ic / self.max_sim_time * 365 * 24 * 60 * 60)
            ...
            results.at["Usage Hours per Year Interval (h)", "Imm. Charging"] = usage_per_year_ic_interval

            usage_per_day_ic_interval = round(energy_ic_interval / max_power_ic, 2)
            results.at["Usage Hours per Day Interval (h)", "Imm. Charging"] = usage_per_day_ic_interval
        ...
        if "used_power_smart" in self.power.columns:
            ...
            energy_sc = round((self.power.used_power_smart * (self.power.end_time - self.power.start_time) / 3600).sum())
            results.at["Energy (kWh)", "Smart Charging"] = energy_sc

            energy_sc_interval = round((power_sorted_interval.used_power_smart * (power_sorted_interval.end_time - power_sorted_interval.start_time) / 3600).sum())
            results.at["Energy Interval (kWh)", "Smart Charging"] = energy_sc_interval

            energy_sc_cost_spot = round(((self.power.used_power_smart * (self.power.end_time - self.power.start_time) / 3600) * self.power.price).sum(), 2)
            results.at["Energy Costs (€)", "Smart Charging"] = energy_sc_cost_spot

            energy_sc_cost_spot_interval = round(((power_sorted_interval.used_power_smart * (power_sorted_interval.end_time - power_sorted_interval.start_time) / 3600) * power_sorted_interval.price).sum(), 2)
            results.at["Energy Costs Interval (€)", "Smart Charging"] = energy_sc_cost_spot_interval
            ...
            grid_sc_cost = round(max_power_sc * 66.82 / 365 * self.max_sim_time / (24 * 60 * 60) + energy_sc * 0.0098, 2)
            results.at["Grid Costs (€)", "Smart Charging"] = grid_sc_cost

            grid_sc_cost_interval = round(max_power_sc * 66.82 / 365 + energy_sc_interval * 0.0098, 2)
            results.at["Grid Costs Interval (€)", "Smart Charging"] = grid_sc_cost_interval

            usage_per_year_sc = round(energy_sc / max_power_sc / self.max_sim_time * 365 * 24 * 60 * 60)
            ...
            results.at["Usage Hours per Year Interval (h)", "Smart Charging"] = usage_per_year_sc_interval

            usage_per_day_sc_interval = round(energy_sc_interval / max_power_sc, 2)
            results.at["Usage Hours per Day Interval (h)", "Smart Charging"] = usage_per_day_sc_interval

        if "used_power_smart" and "used_power_immediately" in self.power.columns:
            results.at["max. Power (kW)", "Comparison [%]"] = round((max_power_sc / max_power_ic - 1) * 100, 2)
            results.at["Energy (kWh)", "Comparison [%]"] = round((energy_sc / energy_ic - 1) * 100, 2)
            results.at["Energy Interval (kWh)", "Comparison [%]"] = round((energy_sc_interval / energy_ic_interval - 1) * 100, 2)
            results.at["Energy Costs (€)", "Comparison [%]"] = round((energy_sc_cost_spot / energy_ic_cost_fix - 1) * 100, 2)
            results.at["Energy Costs Interval (€)", "Comparison [%]"] = round((energy_sc_cost_spot_interval / energy_ic_cost_fix_interval - 1) * 100, 2)
            # results.at["Energy Costs Fix Price (€)", "Comparison [%]"] = round((energy_sc_cost_fix / energy_ic_cost_fix - 1) * 100, 2)
            results.at["Grid Costs (€)", "Comparison [%]"] = round((grid_sc_cost / grid_ic_cost - 1) * 100, 2)
            results.at["Grid Costs Interval (€)", "Comparison [%]"] = round((grid_sc_cost_interval / grid_ic_cost_interval - 1) * 100, 2)
            results.at["Usage Hours per Year (h)", "Comparison [%]"] = round((usage_per_year_sc / usage_per_year_ic - 1) * 100, 2)
            results.at["Usage Hours per Year Interval (h)", "Comparison [%]"] = round((usage_per_year_sc_interval / usage_per_year_ic_interval - 1) * 100, 2)
            results.at["Usage Hours per Day Interval (h)", "Comparison [%]"] = round((usage_per_day_sc_interval / usage_per_day_ic_interval - 1) * 100, 2)
        ...
        for vehicle_id in self.vehicles_id:
            ...
            if vehicle_id in dict_of_pd_plot:
                for index, row in dict_of_pd_plot[vehicle_id].iterrows():
                    vehicle_periods[vehicle_id]["xranges"].append((row.start, row.end - row.start))
    ...
    def validation(self):
        ...
        if (self.precondition_schedule["time_ends"] - self.precondition_schedule["time_start"]).min() < 0 and \
                (self.precondition_schedule["time_ends"] - self.precondition_schedule["time_start"]).max() > 1800:
            print("Precondition duration is higher than configured")
            valid = False

        # Check precondition energy
        precon_energy = (self.precondition_schedule["time_ends"] - self.precondition_schedule["time_start"]) * self.precondition_schedule["charging_power"]
        precon_energy.sum()
        ...
        if total_energy_real > results.at["Energy (kWh)", "Smart Charging"]:
            print("Violation of energy conservation rate. The buses loaded with more enegry then the grid was providing. Smart Charging")
            valid = False

        # Check energy_real (energy in charging processes) bigger than sum of energy sorted in intervalls
        if total_energy_real > results.at["Energy (kWh)", "Imm. Charging"]:
            print("Violation of energy conservation rate. The buses loaded with more enegry then the grid was providing. Immediately Charging")
            valid = False

        if 1-abs(results.at["Energy (kWh)", "Imm. Charging"]/results.at["Energy (kWh)", "Smart Charging"]) > 0.01:
        ...
        for vID, df in self.charging_log.items():
            if df["power"].nunique() >= 8:
                print("Bus "+vID+" charged with "+str(df["power"].nunique())+" different power values.")
                valid = False
...
class ControlSmartCharging:
    ...
    def __init__(self, data,start_date,price_data_path, power_limit_grid = 0, charging_efficiency=1):
        """
        ...
        """
        self.smart_charging = eflips.depot.SmartCharging(data,start_date,price_data_path,power_limit_grid,charging_efficiency)
    ...
    def lowest_power(self,accuracy):
        ...
        if not(isinstance(self.power_limit_grid, int)):
            raise Exception("Function (lowest_power) only works if, the power_limit_grid is an integer.")
        ...
        if self.smart_charging.smart_charging_algorithm():
            power_upper_boundary = potential_power_boundary
            potential_power_boundary = power_upper_boundary - (power_upper_boundary - power_lower_boundary) / 2
            deviation = (power_upper_boundary - power_lower_boundary) / power_lower_boundary
        else:
