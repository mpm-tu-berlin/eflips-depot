...
columns_typ = [
    ...
    "discharge_rate",
    "charge_rate",
]  # all data comes from batery dict
...
for vehicle_number, vehicle_value in vehicles.items():  # each vehicle
    ...
    if vehicle_type not in list(
        vehicle_types.index.values
    ):  # check if vehicle Type is already imported
        for data in columns_typ:
            vehicle_types.loc[vehicle_type, data] = vehicle_value["params"][
                "vehicle_type"
            ].params["battery"][
                data
            ]  # writes the data from battery into the pd frame
    ...
    schedule.loc[ID, "origin"] = vehicle_value["mission_list"][
        0
    ].root_node.origin.short_name
    schedule.loc[ID, "destination"] = vehicle_value["mission_list"][
        0
    ].root_node.destination.short_name
    ...
    if (
        "pantograph"
        in vehicle_value["params"]["vehicle_type"].params["charging_interfaces"]
    ):
    ...
    item = vehicle_value["log_data"]["specific_consumption_primary"]["values"].popitem(
        last=True
    )
